...
export function NotificationBell({
  issues, currentUser, theme, onSelectIssue
    ...
    <>
      ...
      <div className="relative">
        <button
          ...
          className={`relative p-2.5 rounded-2xl border transition-all duration-300 cursor-pointer group ${
            theme === 'dark'
              ? 'bg-white/5 border-white/10 text-gray-300 hover:bg-white/10 hover:text-white'
              : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-900 shadow-sm'
          }`}
        >
          <Bell className="w-5 h-5 group-hover:animate-wiggle transition-transform" />
          {unreadCount > 0 && (
            <motion.span
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="absolute -top-1 -right-1 w-4.5 h-4.5 rounded-full bg-rose-500 border-2 border-slate-900 flex items-center justify-center text-[9px] font-black text-white leading-none"
            >
