...
export default function App() {
  ...
  // Monitor window resize reactively
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    let hasToken = params.get('token') || window.location.pathname.includes('reset-password');
    if (!hasToken && window.location.hash) {
    ...
    let savedToken = null;
    try {
      savedToken = localStorage.getItem('pending_reset_token');
    } catch (e) {}

    if (hasToken || savedToken) {
      // If we are in password reset flow, clear any stale logged-in session!
      try {
        localStorage.removeItem('civic_hero_session');
      } catch (e) {}
      setCurrentUser(null);
      setFbLoading(false);
      // Clear backend session to prevent mismatch
  ...
  // Listen for the custom logout event to instantly clear user session state across components
  useEffect(() => {
    const handleLogoutEvent = () => {
      setCurrentUser(null);
    };
    window.addEventListener('civic-logout', handleLogoutEvent);
    return () => window.removeEventListener('civic-logout', handleLogoutEvent);
  }, []);
  ...
  useEffect(() => {
    ...
    if (tokenParam) {
      try {
        localStorage.setItem('pending_reset_token', tokenParam);
      } catch (e) {
        console.error('Failed to store pending reset token:', e);
      }
    }
    ...
    if (tokenParam || savedToken || window.location.pathname.includes('reset-password')) {
  ...
  return (
    <div className={`min-h-screen font-sans antialiased transition-all duration-500 overflow-x-hidden relative theme-${theme} ${
      ...
      {/* ── MOBILE HEADER PRESERVER ── */}
      {/* The mobile header is now beautifully unified into the sticky page header below, eliminating double headers and offering theme toggles & notifications on desktop! */}
      ...
      <motion.div
        animate={{ paddingLeft: windowWidth >= 1024 ? (sidebarExpanded ? 220 : 64) : 0 }}
        transition={{ type: 'spring', stiffness: 320, damping: 30 }}
        className="flex flex-col flex-1 w-full relative z-10 min-h-screen"
      >
        {/* ── UNIFIED RESPONSIVE HEADER BAR ───────────────────────── */}
        <div className={`sticky top-0 z-40 w-full backdrop-blur-xl border-b transition-all duration-300 ${
          theme === 'dark'
            ? 'bg-slate-950/80 border-white/10'
            : 'bg-white/90 border-slate-200'
        }`}>
          <div className="w-full max-w-full px-4 lg:px-6 py-3 flex flex-col gap-3">
            {/* Main row: Brand/View info on left, action controls on right */}
            <div className="flex items-center justify-between gap-4">
              
              {/* Left side: Brand Logo & Title (Mobile) OR Active Tab Title (Desktop) */}
              <div className="flex items-center gap-2.5">
                {/* On mobile, show brand logo + title. On desktop, show section title */}
                <div className="lg:hidden flex items-center gap-2.5">
                  <div className="w-8 h-8 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-lg flex items-center justify-center">
                    <Sparkles className="w-5 h-5 text-white" />
                  </div>
                  <div>
                    <p className={`text-sm font-black font-display leading-tight ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Samadhan Setu</p>
                ...
                <div className="hidden lg:flex items-center gap-3">
                  <div className="w-9 h-9 bg-gradient-to-br from-indigo-500/10 to-cyan-500/10 border border-indigo-500/20 rounded-xl flex items-center justify-center text-indigo-500 dark:text-indigo-400">
                    ...
                    {activeTab === 'report' && <Sparkles className="w-5 h-5" />}
                    {activeTab === 'authority' && <Shield className="w-5 h-5" />}
                    {activeTab === 'leaderboard' && <Trophy className="w-5 h-5" />}
                    {activeTab === 'dashboard' && <BarChart3 className="w-5 h-5" />}
                  ...
                  <div>
                    <h1 className={`text-base font-black font-display tracking-tight leading-none ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
                      {activeTab === 'map' && 'Explore Radar Map'}
                      {activeTab === 'feed' && 'Civic Feed & Discussions'}
                      {activeTab === 'report' && 'Log Civic Hazard'}
                      {activeTab === 'authority' && 'SLA Dispatch Console'}
                      {activeTab === 'leaderboard' && 'Hero Impact Center'}
                      {activeTab === 'dashboard' && 'SLA Analytical Dashboard'}
                    ...
                    <p className="text-[10px] text-gray-400 dark:text-gray-500 font-medium mt-0.5">
                      {activeTab === 'map' && 'Real-time spatial telemetry and regional issue routing.'}
                      {activeTab === 'feed' && 'SLA tracker, public community board, and local citizen live chat.'}
                      {activeTab === 'report' && 'Report safety threats, potholes, waste overflow, or broken lights.'}
                      {activeTab === 'authority' && 'Authorized municipal workflow controls, assignments, and resolution clocks.'}
                      {activeTab === 'leaderboard' && 'Decentralized gamified rewards, active badges, and citizen impact.'}
                      {activeTab === 'dashboard' && 'Live SLA compliance, average resolution times, and departmental analytics.'}
              ...
              {/* Right side: Global actions & tools */}
              <div className="flex items-center gap-2">
                ...
                <NotificationBell 
                  ...
                  theme={theme}
                  onSelectIssue={(id) => { 
                    const f = displayedIssues.find(i => i.id === id); 
                    if (f) handleSelectIssue(f); 
                  }}
                />
            ...
            {/* Sandbox Banner Tray (integrated elegantly directly in header, visible on all viewports!) */}
                ...
                <div className="flex flex-col md:flex-row gap-3 items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="text-[11px] flex flex-wrap items-center gap-1.5">
                      <span className={`font-bold uppercase tracking-wider text-[9px] ${theme === 'dark' ? 'text-emerald-400' : 'text-emerald-600'}`}>LIVE NETWORK</span>
                      ...
                      {currentUser.role === 'citizen' && (
                        <span className={`font-mono text-[9px] opacity-80 ${theme === 'dark' ? 'text-gray-400' : 'text-slate-500'}`}>
                  ...
                  <div className="flex flex-wrap gap-2 items-center justify-center md:justify-end w-full md:w-auto">
                    <select
                      ...
                      className={`text-[9px] px-2 py-1 rounded-lg border font-black uppercase tracking-wider focus:outline-none cursor-pointer ${
                        theme === 'dark' ? 'bg-indigo-950 border-white/10 text-white' : 'bg-white border-slate-200 text-slate-800'
                      }`}
                    ...
                    <button onClick={handleToggleRole}
                      className="text-[9px] font-black uppercase tracking-wider px-2.5 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white shadow transition-all flex items-center gap-1 cursor-pointer">
                      <Layers className="w-3 h-3" /> Toggle Identity
                    </button>
                ...
                <div className="flex flex-col md:flex-row gap-3 items-center justify-between">
                  ...
                  <div className="flex flex-wrap gap-2 items-center justify-center md:justify-end w-full md:w-auto">
                    ...
                    <button onClick={() => setShowAuthModal(true)}
                      className="text-[9px] font-black uppercase tracking-wider px-3 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white shadow transition-all flex items-center gap-1 cursor-pointer shrink-0">
                      Sign In / Register
                    </button>
                  </div>
                </div>
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4 overflow-y-auto animate-fadeIn">
          <div className={`relative w-full max-w-md rounded-2xl shadow-2xl p-1 overflow-y-auto max-h-[94vh] animate-scaleIn border ${
            theme === 'light' ? 'bg-white border-slate-200 text-slate-900' : 'bg-slate-900 border-white/10 text-white'
          }`}>
            <button 
              onClick={() => setShowAuthModal(false)}
              className={`absolute top-4 right-4 p-1.5 rounded-xl transition-all z-50 cursor-pointer ${
                theme === 'light' 
