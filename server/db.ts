...
export async function addSimulatedNotification(notification: SimulatedNotification): Promise<void> {
  if (isLocalMode) {
    const list: SimulatedNotification[] = readJsonFile(notificationsFile) || [];
    list.unshift(notification);
    writeJsonFile(notificationsFile, list.slice(0, 50));
    return;
  }
  try {
    await db.collection('simulated_notifications').doc(notification.id).set(notification);
  } catch (err: any) {
    if (isFirestoreNotFoundError(err)) {
      enableLocalModeFallback();
      const list: SimulatedNotification[] = readJsonFile(notificationsFile) || [];
...
export async function getSimulatedNotifications(): Promise<SimulatedNotification[]> {
  if (isLocalMode) {
    return readJsonFile(notificationsFile) || [];
  }
  try {
    const snapshot = await db.collection('simulated_notifications')
      .limit(20)
      .get();
    const list: SimulatedNotification[] = [];
    snapshot.forEach(doc => {
      list.push(doc.data() as SimulatedNotification);
    });
    // Sort in memory to guarantee correct ordering without requiring a Firestore index
...
export async function clearSimulatedNotifications(): Promise<void> {
  if (isLocalMode) {
    writeJsonFile(notificationsFile, []);
    return;
  }
  try {
    const snapshot = await db.collection('simulated_notifications').get();
